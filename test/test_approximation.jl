using BasisFunctions, FrameFun, StaticArrays, Domains, CompactTranslatesDict, BasisFunctions.Test, CompactApproximation
...
@testset "Spline approximation" begin
    ...
    x = AZS*b
    @test norm(a*x-b)+1≈ 1
    x = azs_solve(fplatform, i, f2d; cutoff=epsilon)
    @test norm(a*x-b)+1≈ 1
    ...
    x = azs_solve(fplatform, i, fun)
    @test 1+norm(A*x-b)≈1

    op = AZSSolver(fplatform, i)
    ...
    x = AZS*b
    @test norm(a*x-b)+1≈ 1
    x = azs_solve(fplatform, i, f1d; cutoff=epsilon)
    @test norm(a*x-b)+1≈ 1
    op = AZSSolver(fplatform, i)
    x = op*b;@test 1+norm(a*x-b)≈1
...
#
#     b = S*fun
#     AZSS = AZSSolver(A, Zt, BR', DMZ_R;afirst=false)
#     x = AZSS*b
#     @test 1+norm(A*x-b)≈1
#
#     op = AZSSolver(fplatform, i;afirst=false)
#     x = op*b;@test 1+norm(A*x-b)≈1
#
#     x = azs_solve(b, A, Zt, BR', DMZ_R;afirst=false)
#     @test 1+norm(A*x-b)≈1
#
#     x = azs_solve(fplatform, i, fun;afirst=false)
#     @test 1+norm(A*x-b)≈1
#
#     # # 1D
#     f1d = identity
...
#     x = AZS*b
#     @test norm(a*x-b)+1≈ 1
#     x = azs_solve(fplatform, i, f1d; cutoff=epsilon,afirst=false)
#     @test norm(a*x-b)+1≈ 1
#     op = AZSSolver(fplatform, i, afirst=false)
#     x = op*b;@test 1+norm(a*x-b)≈1
#
